...
namespace ElmahLogAnalyzer.UnitTests.Integrations.HttpUserAgentResolving
{
	[TestFixture]
	public class UserAgentStringClientInformationResolverTests : UnitTestBase
	{
		...
		public void Resolve_HttpUserAgentIsEmpty_ReturnsEmptyClientInformation()
		{
			...
			webrequest.Verify(x => x.Uri(It.IsAny<NetworkConnection>()), Times.Never());
		...
		public void Resolve_NotInCache_MakeWebRequestAndAddToCache()
		{
			...
			webrequest.Setup(x => x.Uri(It.IsAny<NetworkConnection>())).Returns(response);
